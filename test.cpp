...
int main()
{
    ...
    // ------------------------------------
    Shader ourShader("VertShade.vs", "fragShade.fs");
    ...
    float offset = 0.0f;
    glm::mat3 trans;
    for (unsigned int height = 0; height < 10; height++) {
        for (unsigned int col = 0; col < size; col++) {
            for (unsigned int row = 0; row < size; row++) {
                cubePositions[k] = glm::vec3(row+offset, height, col+offset);
                k++;
            }
        }
        size--;
    ...
    glm::vec3 spinCube[] = {
        glm::vec3(1.0f+offset, 10.0f, 1.0f+offset)
    };
    ...
    // The FileSystem::getPath(...) is part of the GitHub repository so we can find files on any IDE/platform; replace it with your ownimage path.
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        /*unsigned int modelLoc = glGetUniformLocation(ourShader.ID, "trans");
